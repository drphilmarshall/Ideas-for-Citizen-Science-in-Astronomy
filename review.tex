...
A classification of potential post-merger galaxies selected from the main Galaxy Zoo sample was 
followed by the ability to explore these classifications and the metadata for these galaxies which
is available from the Sloan Digital Sky Survey. A dashboard\footnote{http://tools.zooniverse.org/#/dashboards/galaxy_zoo} allowed
volunteers to explore the data. Thousands of users (around 20\% of those who participated in the classification stage) 
and discussion led to the formulation of a set of astrophysical interesting conclusions; a small number (~10) of users 
collaborated on writing a paper which will be submitted shortly. These later stages required intensive support from
professional scientists (and it is constraints on their time that have prevented early submission of the paper). 

Quench demonstrated that a scaffolded approach, with simple tasks leading to more advanced analysis, can be successful
in encouraging large numbers of volunteers to move beyond simple classification; the number participating by manipulating
data is much higher as a percentage of participants than in Planet Hunters. However, once engagement with the 
literature (either by reading or writing) is required there remains no substitute for significant involvement by professionals. 
