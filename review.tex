...
\todo{Phil}{\#22 Finish off Lang and Hogg Comet Holmes review.}
...
\CaseStudy{Time domain astronomy: Supernova Zoo and Planet Hunters}\sec{SNZoo}
...
\todo{Chris}{\#39 Review Richards and Bloom's contribution.}
\todo{Chris}{\#71 Add SN zoo, PTF refs. In bibtex!}
...
The future of significant citizen science in astrophysics depends to some extend on progress in machine learning; if the `technological singularity' first described by John von Neumann \citep{Ulam}, is imminent then there will be little need for either citizen or professional astronomers! A less ambitious astronomical equivalent - the point at which human review of data is no longer necessary - has been forecast for decades, but as we have described above the number of problems for which manual review of images or data is still carried out is enormous. Even if the proportion of data for which human inspection is necessary decreases dramatically over the next decade, the continued growth in the size of astronomical datasets should ensure that there remains plenty for citizen scientists to do. This section discusses what seems to be the likely path citizen science will take, and we also try and identify some key challenges, and possible limitations, that those planning projects are likely to have to
face.

Consider the example of optical transients. The LSST system paper \citet{LSSTsystem} gives a conservative estimate of $10^5-10^6$ alerts a night. If only 1\% of these require human classification, then that still might produce 10,000 images requiring inspection each night, or roughly one every 100 seconds. Given the increased reliability and likelihood of serendipitous discovery provided by review by citizen scientists, we should take seriously the incorporation of open inspection into plans for LSST transients. Similar arguments (with large error bars) can be made for other surveys, including the vast amounts of data expected to be generated by the Square Kilometer Array; review of transients for LOFAR already requires some human intervention. 

Implicit in this way of thinking is a division of task between human and machine classifiers, something with the majority of projects to data have been able to avoid. Galaxy Zoo and similar projects err instead on the side of including the widest possible range of data, in order to make the task of combining classifications simpler; once the data volume or rate grows further, this must be sacrificed for efficiency's sake. A simple example of human/machine task allocation was mentioned in section \sec{SNzoo}, where analysis of PTF images identified those images which had candidate supernovae which could be inspected by volunteers. It is worth noting that the inclusion of human inspection changed the nature of the machine learning task; instead of prioritising fidelity (producing a small but accurately classified set of candidates), the task for machine learning is to identify a subset of the images which contains many false positives but also a complete set of all supernovae. 
